defmodule Project2Bonus do
  ...
  defp parse_args (args)do
    ...
    numofError = String.to_integer(Enum.at(k,3))
    ...
    if (topology=="3Dtorus") do
      ...
      allNodes = Enum.map((1..numNodes), fn(x) ->
        ...
        allNodes=occurError(allNodes,0,numofError)
  ...
  def occurError(allNodes,counter,numofError) do
    errorNode =Enum.random(allNodes)
    newallNodes=List.delete(allNodes,errorNode)
    if counter<(numofError-1) do
      occurError(newallNodes,counter+1,numofError)
    else
      newallNodes
    end
  end
  ...
  def periodicGossipTransmission(adjacentList, startTime, allNodes) do
    ...
    Task.start(Project2Bonus,:transmitGossip,[chosenRandomAdjacent, startTime, allNodes])
    #:timer.sleep 2
